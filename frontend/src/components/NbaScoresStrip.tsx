...
import { api, type NbaGame, type NbaScoresResponse } from "../lib/api";
import { PlayoffSeriesStats } from "./PlayoffSeriesStats";

const POLL_MS = 20000;

function stateLabel(g: NbaGame): string {
...
function stateTone(g: NbaGame): string {
  const s = (g.state || "").toLowerCase();
...
export function NbaScoresStrip() {
  const [data, setData] = useState<NbaScoresResponse | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await api.nbaScores();
        if (!alive) return;
        setData(res);
        setError(res.error);
      } catch (err) {
        if (!alive) return;
        setError(err instanceof Error ? err.message : "nba scores failed");
      }
    }
    load();
  ...
  if (error) return <div className="text-term-red text-xs">nba: {error}</div>;
  if (!data) return <div className="text-term-dim text-xs">loading NBA scores…</div>;
  if (games.length === 0) return <div className="text-term-dim text-xs">no NBA games today</div>;
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-1">
              ...
              {g.series_summary && (
                <span className="text-[9px] text-term-dim truncate ml-2">{g.series_summary}</span>
              )}
            </div>
